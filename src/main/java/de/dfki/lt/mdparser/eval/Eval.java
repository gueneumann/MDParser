...
public class Eval {
      ...
      if (!((lineGold.length() > 0)
        ...
        if (lineGold.length() > 0) {
          ...
          if (!parsedArray[headIndexParsed].equals("_")) {
            //System.out.println(parsedArray[headIndexParsed]+" "+(goldArray[headIndexGold]
            //+" --> "+parsedArray[1]+" "+goldArray[1]));
  ...
  private void createLabelIndexMap(String trainingDataFile, int labelIndexGold) throws IOException {
    ...
    while ((lineGold = frGold.readLine()) != null) {
      if (lineGold.length() > 0) {
        ...
        if (this.labelIndexMap.get(label) == null) {
          this.labelIndexMap.put(label, curLabelIndex);
          curLabelIndex++;
        }
      }
    }
    // GN: added this
    frGold.close();
    this.labelIndexMap.put("mk1", curLabelIndex);
    curLabelIndex++;
    this.labelsArray = new String[this.labelIndexMap.size()];
    Iterator<String> iter = this.labelIndexMap.keySet().iterator();
    while (iter.hasNext()) {
  ...
  private void computeLabelConfusionMatrix(String goldStandardFile, String parsedFile, int headIndexGold,
    ...
    BufferedReader frParsed = new BufferedReader(ir2);
    String lineParsed;
    int[][] confusionMatrix = new int[this.labelIndexMap.size()][this.labelIndexMap.size()];
    while ((lineGold = frGold.readLine()) != null) {
      lineParsed = frParsed.readLine();
      if (lineGold.length() > 0) {
        String[] parsedArray = lineParsed.split("\\s");
        ...
        int curLabelGoldIndex = this.labelIndexMap.get(curLabelGold);
        int curLabelParsedIndex = this.labelIndexMap.get(curLabelParsed);
