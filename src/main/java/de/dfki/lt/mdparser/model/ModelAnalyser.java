...
public class ModelAnalyser {
  ...
  public void editTrainingData() throws IOException {
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      edit(curFile, this.oldAlphabetFile, this.newAlphabetFile);
  ...
  public HashMap<String, Double> computeUsefulness(String modelFileDir) throws IOException {
    ...
    Alphabet alpha = new Alphabet(this.newAlphabetFile);
    ...
    files = new File(this.trainingDataDir).listFiles();
    int c = 0;
    String mode = "2";
    for (int i = 0; i < files.length; i++) {
      File curFile = files[i];
      ...
      while ((line = fr.readLine()) != null) {
        if (line.length() > 0) {
          ...
          for (int k = 1; k < trainingInstance.length; k++) {
            Integer index = Integer.valueOf(trainingInstance[k].split(":")[0]);
            //System.out.println(k+" "+weightsMap+" "+curFile.getName()+" "+curWeights+" "
            //+index+" "+numberOfClasses+" "+labelIndex);
