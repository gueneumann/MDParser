...
* a problem is that from the sequence of daughters, I do not know where to insert in the head
* element that means: where to cut the daughter sequence.
...
* I could also search for the head index, by searching the word form of the head in left to right
* order (and also remember, which index has been used - in case a word occurs several times).
...
*
* NOTE: basically this means that I have to define an aligner, but can assume that has an implicit
* order since it was created automatically from a sequence.
*
* (_RT intends (_SBJ bill (_NMOD The )_NMOD )_SBJ (_OPRD to (_IM restrict
* (_OBJ RTC (_NMOD the )_NMOD )_OBJ
* (_ADV to (_PMOD borrowings (_NMOD Treasury )_NMOD (_NMOD only )_NMOD )_PMOD )_ADV )_IM )_OPRD
* (_P , )_P (_ADV unless (_SUB receives (_SBJ agency (_NMOD the )_NMOD )_SBJ
* (_OBJ authorization (_NMOD specific )_NMOD (_NMOD congressional )_NMOD )_OBJ
* )_SUB )_ADV (_P . )_P )_RT
*
* The|DT bill|NN intends|VBZ to|TO restrict|VB the|DT RTC|NNP to|TO Treasury|NNP borrowings|NNS
* only|RB ,|, unless|IN the|DT agency|NN receives|VBZ specific|JJ congressional|JJ
* authorization|NN .|.
*
...
public class DeLinearizedSentence {
  ...
  /**
   * This is used to define a mapping from a word to its different mentioning positions in a
   * sentence. The order in the stack/deque is from left to right.
   *
   * @param word
   * @param index
   */
  ...
  /**
   * Receives a wordPosSequence of tokens of form word|POS and creates an initial Conll sentence
   * object with filled columns for index, word, pos. As a side effect, it creates a mapping from
   * word to all possible indexes in the sentence, e.g., the word "a" can occur several times in a
   * sentence at several positions, so we insert the indices in a stack from left to right.
   */
  ...
  */
      ...
      */
      if (elem.startsWith("(_")) {
        ...
        */
        ...
        /*
        System.out.println("Index: " + wordIndex + " Word: " + elem + " First stack: "
            + this.headIdStack.getFirst());
        */
        this.addHeadIdLabeltoWordIndex(wordIndex, this.headIdStack.getFirst(),
            this.labelStack.getFirst());
  ...
  /**
   * Receives a pair of aligned wordPos-token sequenc and linearized dependency tree and returns a
