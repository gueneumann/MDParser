...
public abstract class ParsingAlgorithm {
	...
	//TEST-GDS-COMBINED
	public abstract void processCombined(Sentence sent, FeatureModel fm, boolean noLabels, HashMap<String,String> splitMap);
	...
	public synchronized String findTheMostFreqLabel(String pos) {
		Iterator<String> iter = labelFreqMap.keySet().iterator();
	...
	public synchronized void increaseCount(String key) {
		Integer curKeyFreq = labelFreqMap.get(key);
		if (curKeyFreq == null) {
			labelFreqMap.put(key, 1);
		}
		else {
			labelFreqMap.put(key, 1+curKeyFreq);
		}	
	}
