...
public class CovingtonFeatureModel extends FeatureModel {
  ...
  public CovingtonFeatureModel(Alphabet alphabetParser, FeatureExtractor fe) {
    ...
    // WHY 11
    this.mergeFeatureNames = new String[11];
  ...
  public void initializeStaticFeaturesCombinedOrig(Sentence curSent, boolean train) {
    ...
    for (int j = 1; j < curSentArray.length + 1; j++) {
      ...
      Feature fpjp1_fpjp2_fpjp3 = fe.merge3(0, this.mergeFeatureNames, fpjp1, fpjp2, fpjp3);
    ...
    for (int i = 0; i < curSentArray.length + 1; i++) {
      ...
      Feature fpi_fpip1 = fe.merge2(1, this.mergeFeatureNames, fpi, fpip1);
      addStaticFeature(iList, fpi_fpip1, alphaParser);
      Feature fwfi_fcpi = fe.merge2(2, this.mergeFeatureNames, fwfi, fcpi);
  ...
  public FeatureVector applyCombined(ParserState state, boolean train, boolean noLabels) {
    ...
    Feature fdepri_fpj = fe.merge2(3, this.mergeFeatureNames, fdepldj, curStaticFeaturesJ[0]);
    fvParser.addFeature(fdepri_fpj, alphaParser, train);

    // curStaticFeaturesI[0]: should be pi -> OK
    Feature fpi_fdeprdi_fdepldi = fe.merge3(4, this.mergeFeatureNames, curStaticFeaturesI[0], fdeprdi, fdepldi);
    fvParser.addFeature(fpi_fdeprdi_fdepldi, alphaParser, train);

    Feature fdepi_wfhi = fe.merge2(5, this.mergeFeatureNames, fdepi, wfhi);
    fvParser.addFeature(fdepi_wfhi, alphaParser, train);

    // curStaticFeaturesJ/I[1]: should be pjp1 and pip1 -> OK
    Feature fphi_fpjp1_fpip1 = fe.merge3(6, this.mergeFeatureNames, fphi, curStaticFeaturesI[1], curStaticFeaturesJ[1]);
    ...
    Feature fst2 =
        fe.merge3(7, this.mergeFeatureNames, curStaticFeaturesJ[4], curStaticFeaturesI[2], curStaticFeaturesJ[3]);
    ...
    Feature fst3 = fe.merge3(8, this.mergeFeatureNames, dist, curStaticFeaturesJ[0], curStaticFeaturesJ[5]);
