...
public class StanfordOutput {
	...
	public StanfordOutput(String inputString) throws IOException {
		...
		for (int i = 0; i < sentences.size();i++) {
			...
			if (parsedSentence.length > 2) {
				...
				for (int k=0; k < parsedSentence.length;k++) {			
					...
					if (Integer.valueOf(parentIndex)!= 0 && !label.equals("ROOT") && !label.equals("Unk") && !label.equals("PUNCT")) {
		...
		parsedOutput = sbp.toString();
		taggedOutput = sbt.toString();
	}
